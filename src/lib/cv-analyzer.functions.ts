...
function buildPrompt(cvText: string, jd: string, industry: string, mode: string) {
    ...
    const res = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: buildPrompt(data.cvText, data.jobDescription, data.industry, data.mode) },
        ],
...
export type CvAnalysis = {
  ...
  sections: { name: string; current: string; feedback: string; rewrite: string }[];
  bulletOptimizer: {
    before: string;
    variants: { strategic: string; metrics: string; consulting: string; executive: string };
  }[];
  keywords: { found: string[]; missing: string[]; suggestions: string[] };
  recommendations: string[];
  recruiterVerdict: string;
  heatmap: { section: string; strength: "strong" | "medium" | "weak"; note: string }[];
  optimizedCv: {
    fullName: string;
    title: string;
    contact: { email: string; phone: string; location: string; linkedin: string };
